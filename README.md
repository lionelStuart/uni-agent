...
- **本地记忆夹**：**`memory_index.json`** 存 **L0** 与 **`l1/*.json`** 路径；**`memory_search`**（工具与 REPL `memory search`）在配置 LLM 时：**由模型根据问题生成关键词 → 在 L0 上做子串匹配 → 将命中条目的 L1 交给模型整理回答**；关闭 `UNI_AGENT_MEMORY_SEARCH_USE_LLM` 时退化为 L0 字面子串检索
