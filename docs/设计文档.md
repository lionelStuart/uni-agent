...
外部 best practice 与 harness / loop 预研见：[Agent研发预研与最佳实践.md](./Agent研发预研与最佳实践.md)。
...
- **Sub-agent**：当计划执行 `delegate_task` 时，在当前 `run_id` 上下文中启动**子** `Orchestrator.run`（子侧不注册 `delegate_task`，单层）；子结果格式化回父步骤 `output`，并产生独立子 run 落盘记录（含 `parent_run_id`）。
...
- **委派步骤**：内置 `delegate_task` 用于「单独一轮子 agent」类子目标；除 LLM 选用与静态计划外，启发式与 Pydantic 规划在 **`memory_search` 短路之后** 可对用户任务中的 **显式委派关键词**（`delegate_task`、`子代理`、`sub-agent`、`subagent`）直接生成单步 `delegate_task`（实现见 `agent/planner.py`、`agent/pydantic_planner.py`）。
...
- `http_fetch`
- `search_workspace`
- `command_lookup`（PATH 解析、`--help` 抓取或前缀列举）
- `run_python`（工作区内短片段，产物在 `.uni-agent/code_run/`）
- `memory_search`（本地 L0/L1；可选 LLM 关键词与整理）
